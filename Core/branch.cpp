...
namespace RV32IM
{
	...
	BranchPrediction::BranchPrediction(const size_t& memory_size) : branch_status_table(new uint8_t[memory_size]), branch_history_table(new uint8_t[memory_size])
	{
		memset(branch_status_table.get(), 0, sizeof(uint8_t) * memory_size);
		memset(branch_history_table.get(), 0, sizeof(uint8_t) * memory_size);
	}

	bool BranchPrediction::take_branch(const unsigned_data address) const
	{
		// 00 strong no branch
		// 01 weak no branch
		// 10 weak branch
		...
		return branch_status_table[address + (branch_history_table[address] & 0b11)] >= 2;
	}

	void BranchPrediction::update_table(const unsigned_data address, const bool branch_taken)
	{
