...
namespace RV32IM
{
	...
	{
		return *(reinterpret_cast<uint32_t*>(memory.get() + address));
	}

	inline uint16_t UnifiedMemory::read_half_word(uint32_t address) const
	{
		return *(reinterpret_cast<uint16_t*>(memory.get() + address));
	}
	...
	{
		*(reinterpret_cast<uint32_t*>(memory.get() + address)) = data;
	}

	inline void UnifiedMemory::write_half_word(uint32_t address, uint16_t data) const
	{
		*(reinterpret_cast<uint16_t*>(memory.get() + address)) = data;
	}
